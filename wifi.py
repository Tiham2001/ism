...
# Helper functions
def in_sudo_mode():#user runs cmd with sudo
...
def backup_csv():#remove csv file before we rub our sc
...
def check_for_essid(essid, lst):#test to c essid in list, if exist it will ignore, else it will add it the list
...
def wifi_networks_menu():
    ...
    active_wireless_networks = list()# empty list to store network devices
    try:
        while True:
...
print("\n****************************************************************")
print("\n* Copyright of Rajesh, Tiham, Yogavarshni:  2023                              *")
print("\n****************************************************************")
...
subprocess.run(["airmon-ng", "start", wifi_name, hackchannel])#KILL THE NETWORK MANAGERS
try:
    while True:
        count = 0
